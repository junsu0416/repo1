...
public class CheckFilter implements Filter {
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException {
		...
		long before = System.currentTimeMillis();	// 얼마나 걸린지 시간 확인용
	    ...
	    // Filter 의 doFilter에서의 전달인자는 우리가 사용했던 
	    // HttpServletRequest 를 부모형태로 캐스팅한 변수를 사용하고 있어서 
	    // 필요한 메소드 사용을 위해 다시 자식 형태로 형변환
	    HttpServletRequest req = (HttpServletRequest)request;	// 전달인자가 달라서 형변환 
	    HttpServletResponse resp = (HttpServletResponse)response;
	    
	    System.out.println("getServletPath : " + req.getServletPath());	// 주소를 반환하는 
	    ...
	    if(isExclude(req.getServletPath())) {	// true 인지 false 인지 
	    	...
	    	if(isLogon != null && isLogon == true) {
	    		System.out.println("로그인 완료 통과");
	    		chain.doFilter(request, response);
	    	} else {
	    		System.out.println("로그인 실패 통과못함");
	    		// 그렇지 않으면 다른 페이지로 이동 
