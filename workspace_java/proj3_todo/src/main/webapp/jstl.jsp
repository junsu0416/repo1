...
<c:set var="a2" value="${id }" scope="page" />
	...
	scope 생략시 page
<c:set var="a3" value="로그인" /> <!-- scope 생략시 자동으로 page로 -->
<c:set var="a4" value="로그인" scope="session"/>	<!-- session 이기 떄문에 30분 동안 유지  -->
...
<c:set var="game" value="블루아카이브" scope="application"/>

기본적으로 page -> request -> session -> application 순으로 찾음 <br>
game : ${game }<br>
pageScope : ${pageScope.game }<br>	<!-- 어디 스코프인지 적어서 그 값을 꺼내올 수 있음 -->
...
end가 items 의 개수보다 크면 items 끝까지만 돌고 에러 없음 <br>
